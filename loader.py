...
from chemutils import brics_decomp, get_clique_mol, get_mol
from itertools import repeat, product, chain
import random

# allowable node and edge features
allowable_features = {
    'possible_atomic_num_list': list(range(1, 119)),  # 118  [1-118] value:0-117
...
def mol_to_graph_data_obj_motif(mol):
    """
    ...
    super_x = torch.tensor([[121, 0]]).to(x_nosuper.device)
    ...
    if num_motif > 0:
        ...
        motif_edge_attr[:, 0] = 6  # bond type for motif edge
        ...
        super_edge_attr[:, 0] = 7  # bond type for graph edge
    ...
    data.x_nosuper = x_nosuper
    data.edge_index_nosuper = edge_index_nosuper
    data.edge_attr_nosuper = edge_attr_nosuper
    data.num_part = torch.tensor(np.array(num_part), dtype=torch.long)
...
def graph_data_obj_to_mol_simple(data_x, data_edge_index, data_edge_attr):
    ...
    # fails for COC1=CC2=C(NC(=N2)[S@@](=O)CC2=NC=C(
...
class MoleculeDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        if self.dataset == 'zinc_standard_agent':
            ...
            # smiles_list,zinc_id_list = zip(*combined_list[:250000])
            j = 0
            for i in range(len(smiles_list)):
                ...
                if rdkit_mol != None:  # ignore invalid mol objects
                    ...
                    data = mol_to_graph_data_obj_motif(rdkit_mol)
                    # manually add mol id
                    id = int(zinc_id_list[i].split('ZINC')[1].lstrip('0'))
                    data.id = torch.tensor(
                        [i - j])  # id here is zinc id value, stripped of
                    # leading zeros
            ...
            for i in range(len(smiles_list)):
                ...
                if rdkit_mol != None:  # ignore invalid mol objects
                    data = mol_to_graph_data_obj_simple(rdkit_mol)
                    # manually add mol id
                    data.rdkit_index = torch.tensor([i])
                    # print(type(s))
                    # data.smiles = s
                    data.id = torch.tensor(
                        [i - j])  # id here is zinc id value, stripped of
            ...
            for i in range(len(smiles_list)):
                ...
                if rdkit_mol != None:
                    ...
                    # manually add mol id
                    data.id = torch.tensor(
                        [i - j])  # id here is the index of the mol in
                    # the dataset
                    data.y = torch.tensor(labels[i, :])
                    data_list.append(data)
                    data_smiles_list.append(smiles_list[i])
            ...
            for i in range(len(smiles_list)):
                ...
                if rdkit_mol != None:
                    ...
                    data.id = torch.tensor(
                        [i - j])  # id here is the index of the mol in
                    # the dataset
                    # data.smiles = torch.tensor(smiles_list[i])
                    data.y = torch.tensor([labels[i]])
                    data_list.append(data)
                    data_smiles_list.append(smiles_list[i])
                else:
...
class MoleculeDataset2(InMemoryDataset):
    def __init__(self,
                 root,
                 # data = None,
                 # slices = None,
        ...
        super(MoleculeDataset2, self).__init__(root, transform, pre_transform,
                                               pre_filter)
        self.transform, self.pre_transform, self.pre_filter = transform, pre_transform, pre_filter
    ...
    def process(self):
        ...
        if self.dataset == 'zinc_standard_agent1':
            input_path = self.raw_paths[0]
            input_df = pd.read_csv(input_path, sep=',', compression='gzip',
                                   dtype='str')
            smiles_list = list(input_df['smiles'])
