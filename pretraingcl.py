...
import numpy as np
from gnn_model import GNN, GNNDecoder, DiscreteGNN
...
def train(model_list, loader1, optimizer, device, args):
    model, projection_decoder = model_list
    model.train()
    projection_decoder.train()
    train_loss_accum = 0
    for step, batch in enumerate(loader1):
        batch_size = batch.batch_len
        ...
        x1_node_rep = model.forward_cl2(batch.x, batch.edge_index, batch.edge_attr)
        num_part = batch.num_part

        x1_node_rep, x1_motif_node_rep, x1_super_node_rep = group_node_rep(x1_node_rep, batch_size, num_part)
        x1_rep = torch.stack(x1_super_node_rep, dim=0)
        x1_rep = projection_decoder(x1_rep)
...
def main():
    ...
    parser.add_argument('--output_model_file', type=str, default='./saved_model/gclgrad.pth',
    ...
    model_list = [model, projection_decoder]
    optimizer = optim.Adam([{"params": model.parameters()}, {"params": projection_decoder.parameters()}], lr=args.lr,
